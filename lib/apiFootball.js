...
// Fetch a real player headshot from TheSportsDB (free, no key).
// Next.js caches each URL for 7 days so TheSportsDB is only hit once per player per week.
...
// Attach TheSportsDB headshots to a map of {id → player} objects in-place.
// Fires all lookups in parallel (Next fetch cache dedups identical names).
async function attachHeadshots(playerMap) {
  await Promise.all(
    Object.values(playerMap).map(async (p) => {
      const url = await tsdbHeadshot(p.name);
      if (url) p.photo = url;
    })
  );
}
...
export async function getPlayerProfile(playerId) {
  ...
  const photo = await tsdbHeadshot(a.displayName);
  return {
    player: {
      id: a.id,
      name: a.displayName,
...
export async function getLeaders() {
  ...
  // Fetch real headshots from TheSportsDB for all players in parallel.
  // Each result is cached 7 days by Next.js, so TheSportsDB is only hit once per player per week.
  await Promise.all([
    attachHeadshots(playerTotals),
    attachHeadshots(keeperTotals),
    attachHeadshots(cardTotals),
  ]);
