...
// Reflects the REAL results from the live feed. Each game carries explicit
// team codes (h/a) plus `w` (winner code) for every completed match.
//   • R32 → QF are locked (results in). `w` drives advancement + highlight.
//   • Semi-finalists are explicit (h/a) with no `w` yet — scores resolve live.
//   • Final has no teams yet; it chains from the SF winners via srcH/srcA
//     and populates automatically once the semi-finals are played.
// Live scores for any game are fetched by name from the scoreboard at runtime,
// so we only hard-code the teams and the winners, never the scorelines.
// ---------------------------------------------------------------------------
...
// FINAL — no teams yet; chains from the SF winners once they are decided.
export const FINAL = [
