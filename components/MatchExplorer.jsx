            ...
            <>
              {isFinalOrLive ? (
                <>
                  ...
                  {/* AI Match Report — generated by Claude Sonnet */}
                  {result.status === 'final' && (
                    <div className="block ai-report-block">
                      <div className="ai-report-header">
                        <span className="ai-report-label">✦ AI Match Report</span>
                        <span className="ai-report-tag">Claude Sonnet</span>
                      </div>
                      {aiReport ? (
                        <p className="ai-report-text">{aiReport}</p>
                      ) : aiReportLoading ? (
                        <div className="ai-report-loading">
                          <span className="ai-report-dots" />
                          Writing match report…
                        </div>
                      ) : (
                        <div className="ai-report-cta">
                          <p className="ai-report-hint">Get a narrative match report written by Claude — goals, tactics, and what it means for the tournament.</p>
                          {aiReportError && <p className="ai-report-err">Failed to generate. Check your API key or try again.</p>}
                          <button
                            ...
                            onClick={async () => {
                              ...
                              try {
                                const res = await fetch('/api/report', {
                                  ...
                                  body: JSON.stringify({ home: h.n, away: a.n, result }),
                                });
                                const data = await res.json();
                                if (data.report) setAiReport(data.report);
                                else setAiReportError(true);
                              } catch {
